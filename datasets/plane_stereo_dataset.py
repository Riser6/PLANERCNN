...
class PlaneDataset(PlaneDatasetSingle):
    ...
    def __getitem__(self, index):
        ...
        for info in [info_1, info_2]:
            ...
            image = utils.mold_image(image.astype(np.float32), self.config) #(640, 640, 3)
            depth = np.concatenate([np.zeros((80, 640)), depth, np.zeros((80, 640))], axis=0)   #(640, 640)
            ...
            print(depth.astype(np.float32).shape)
            print(extrinsics.astype(np.float32).shape)
        ...
        """print("######################")
        print(np.concatenate([translation, axis, np.array([angle])], axis=0).astype(np.float32).shape)
        print(info_1[1].astype(np.float32).shape)
        print(info_2[1].astype(np.float32).shape)
        print(correspondence.shape)
        print(camera.astype(np.float32).shape)
        print(len(data_pair))"""
        return data_pair
