...
class ScanNetScene():
    ...
    def __init__(self, options, scenePath, scene_id, confident_labels, layout_labels, load_semantics, load_boundary=False):
        ...
        if self.scannetVersion == 1:
            ...
            self.imagePaths = [scenePath + '/color/' + str(imageIndex) + '.jpg' for imageIndex in range(self.numImages - 1)]
            pass
    ...
    def __getitem__(self, imageIndex):
        ...
        if self.scannetVersion == 1:
            ...
            segmentationPath = imagePath.replace('color/', 'annotation/segmentation/').replace('.jpg', '.png')
