...
class Config(object):
    ...
    def applyAnchorsTensor(self, class_ids, parameters):
        if 'joint' in self.ANCHOR_TYPE:
            anchors = self.ANCHOR_PLANES_TENSOR[class_ids.data - 1].to(parameters.device)
            parameters = parameters[:, :3] + anchors
        elif self.ANCHOR_TYPE == 'Nd':
            normals = self.ANCHOR_NORMALS_TENSOR[(class_ids.data - 1) % self.ANCHOR_OFFSETS_TENSOR].to(parameters.device)
            offsets = self.ANCHOR_OFFSETS_TENSOR[(class_ids.data - 1) // self.ANCHOR_OFFSETS_TENSOR].to(parameters.device)
            parameters = (parameters[:, :3] + normals) * (parameters[:, 3] + offsets)
        elif self.ANCHOR_TYPE == 'layout':
            anchors = self.ANCHOR_PLANES_TENSOR[class_ids.data - 1].to(parameters.device)
            parameters = parameters[:, :3] + anchors
        elif 'normal' in self.ANCHOR_TYPE:
            normals = self.ANCHOR_NORMALS_TENSOR[class_ids.data - 1].to(parameters.device)
