...
def train(options):
    ...
    if local_rank != -1:
        ...
        group_model2 = torch.distributed.new_group(ranks=[0,1])
        ...
        refine_model = DDP(refine_model, device_ids=[local_rank], output_device=local_rank,find_unused_parameters=True,process_group=group_model2,broadcast_buffers=True)
    ...
    for epoch in range(options.numEpochs):
        ...
        for sampleIndex, sample in enumerate(data_iterator):
            ...
            for indexOffset in [0, 13]:
                ...
                #print(rpn_class_loss.shape, rpn_bbox_loss.shape)
            ...
            if (len(detection_pair[0]['detection']) > 0 and len(detection_pair[0]['detection']) < 30) and 'refine' in options.suffix:
                ...
                image = (input_dict['image'] + config.MEAN_PIXEL_TENSOR.view((-1, 1, 1)).to(input_dict['image'].device)) / 255.0 - 0.5
                image_2 = (input_pair[1 - c]['image'] + config.MEAN_PIXEL_TENSOR.view((-1, 1, 1)).to(input_pair[1 - c]['image'].device)) / 255.0 - 0.5
                ...
                for resultIndex, result in enumerate(results[1:]):
                    if 'depth' in result:
                        depth_loss = torch.sum(result['depth']) * 0
                    if 'mask' in result:
            ...
            if sampleIndex % 500 < options.batchSize or options.visualizeMode == 'debug' and False:
