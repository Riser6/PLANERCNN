...
def proposal_layer(inputs, proposal_count, nms_threshold, anchors, config=None):
    ...
    ## Replace to torchvision implement
...
def detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, gt_parameters, config):
    ...
    negative_roi_bool = negative_roi_bool & no_crowd_bool.bool()
...
def refine_detections(rois, probs, deltas, parameters, window, config, return_indices=False, use_nms=1, one_hot=True):
    ...
    if use_nms == 2:
        ...
        ## Replace to torchvision implement
        nms_keep = ops.nms(ix_rois, ix_scores, config.DETECTION_NMS_THRESHOLD)
        ...
        for i, class_id in enumerate(unique1d(pre_nms_class_ids)):
            ...
            ## Replace to torchvision implement
            nms_keep = ops.nms(ix_rois, ix_scores.unsqueeze(1), config.DETECTION_NMS_THRESHOLD)
...
class MaskRCNN(nn.Module):
    ...
    def predict(self, input, mode, use_nms=1, use_refinement=False, return_feature_map=False):
        ...
        if self.config.PREDICT_DEPTH:   #True
            depth_np = self.depth(feature_maps) #torch.Size([1, 1, 640, 640])

            if self.config.PREDICT_BOUNDARY:    #False
                boundary = depth_np[:, 1:]
                depth_np = depth_np[:, 0]
            else:
        ...
        ranges = self.config.getRanges(input[-1]).transpose(1, 2).transpose(0, 1).to(input[-1].device)
        zeros = torch.zeros(3, (self.config.IMAGE_MAX_DIM - self.config.IMAGE_MIN_DIM) // 2, self.config.IMAGE_MAX_DIM).to(ranges.device)
        ...
        rpn_class_logits, rpn_class, rpn_bbox = outputs     #torch.Size([1, 102300, 2])
        ...
        rpn_rois = proposal_layer([rpn_class, rpn_bbox],
                                 ...
                                 config=self.config)    #torch.Size([1, 2000, 4])
        ...
        if mode == 'inference':
            ...
            gt_boxes = gt_boxes / scale     #torch.Size([1, 18, 4])
            ...
            rois, target_class_ids, target_deltas, target_mask, target_parameters = \
                detection_target_layer(rpn_rois, gt_class_ids, gt_boxes, gt_masks, gt_parameters, self.config)      #torch.Size([200, 4]) torch.Size([200]) torch.Size([200, 4]) torch.Size([200, 28, 28]) torch.Size([200, 3])
