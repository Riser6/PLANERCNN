...
def unmoldDetections(config, camera, detections, detection_masks, depth_np, unmold_masks=True, debug=False):
    ...
    if 'normal' in config.ANCHOR_TYPE:
        ...
        ranges = config.getRanges(camera).transpose(1, 2).transpose(0, 1).to(camera.device)
        zeros = torch.zeros(3, (config.IMAGE_MAX_DIM - config.IMAGE_MIN_DIM) // 2, config.IMAGE_MAX_DIM).to(ranges.device)        
...
def warpModuleDepth(config, camera, depth_1, features_2, extrinsics_1, extrinsics_2, width, height):
    ...
    XYZ_1 = config.getRanges(camera).to(camera.device) * depth_1[padding:-padding].unsqueeze(-1)
...
def calcXYZModule(config, camera, detections, masks, depth_np, return_individual=False, debug_type=0):
    ...
    ranges = config.getRanges(camera).to(camera.device)
    ranges_ori = ranges
    zeros = torch.zeros(3, (config.IMAGE_MAX_DIM - config.IMAGE_MIN_DIM) // 2, config.IMAGE_MAX_DIM).to(ranges.device)       
